...
class BFGSOptimizer:
    def __init__(self, max_it=100, tolerance=1e-5, rho=0.5):
        self.max_it = max_it
    ...
    def solve(self, func, x0, color, obj_local, lienzo_local):
        ...
        for i in range(self.max_it):
            ...
            # --- CORRECCIÓN CRÍTICA ---
            # Limitamos el tamaño del pincel entre 2 y 15 para evitar manchas gigantes
            xk_next[2] = np.clip(xk_next[2], 2.0, 15.0) 
            xk_next[3] = np.clip(xk_next[3], 2.0, 15.0)
            xk_next[5] = np.clip(xk_next[5], 0.1, 0.6) # Limitar opacidad
